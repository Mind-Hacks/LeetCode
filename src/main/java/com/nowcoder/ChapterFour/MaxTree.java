 ...
 * 现有一建树方法，对于数组中的每个元素，其在树中的父亲为数组中
 * 它左边比它大的第一个数和右边比它大的第一个数中更小的一个。
 * 若两边都不存在比它大的数，那么它就是树根。请设计O(n)的算法实现这个方法。
 ...
 解题思路：
 找出每个元素左右第一个大的数，然后将其中较大的数作为父节点建立MaxTree
 题目要求返回数组，每个元素为父亲节点编号，因此只需要找到每个节点的父节点即可。找每个点父节点的时候需要用到栈结构。
 * @Author:mindHacks
 * @Date:2017年12月18日23:38
 * @wiki:
 */
public class MaxTree {
    ...
    public static void main(String[] args) {
        ...
        int[] maxTree = buildMaxTree(test, 4);
        ArrayUtil.printAyyay(maxTree, maxTree.length);
    }
}
